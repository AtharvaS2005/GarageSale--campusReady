...
import { verifyServerConnectivity } from '../utils/serverCheck';
...
const Login = () => {
  ...
  useEffect(() => {
    verifyServerConnectivity(setError);
  }, []);
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values) => {
      try {
        ...
        const isConnected = await verifyServerConnectivity(setError);
        if (!isConnected) {
        ...
        if (!error.message || error.message === 'Server connectivity check failed') {
          // The error is already set by verifyServerConnectivity
        } else {
